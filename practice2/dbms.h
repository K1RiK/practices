...
int _main(char* result, char stri[]) {
    ...
    if (argc > 2 && strcmp(argv[1], "--file") == 0) {  // Проверяем есть ли флаг --file и его значение
        ...
        snprintf(result, 4096, "%sУкажите флаг --file <имя файла>\n", result);
        return 1;
    }
    ...
    if (argc > 4 && strcmp(argv[3], "--query") == 0) {  // Проверяем есть ли флаг --query и его значение
        ...
        if ((iswoarg && argc == 6) || (!iswoarg && argc == 7) || (strcmp(argv[4], "HSET") == 0 && argc == 8)) {
            ...
            snprintf(result, 4096, "%sУкажите флаг --query <запрос>\n", result);
            return 1;
        }
    } else {
        snprintf(result, 4096, "%sУкажите флаг --query <запрос>\n", result);
    ...
    if (fr == NULL) {
        snprintf(result, 4096, "%sФайл не найден или у программы нет к нему доступа!", result);
        return 1;
    }

    char line[16000];
    char head[64][4096];  // Сохраняем содержимое файла до найденой строки
    char footer[64][4096];  // Сохраняем содержимое файла после строки
